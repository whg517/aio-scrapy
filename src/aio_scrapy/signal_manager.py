...
class SignalManager:
    ...
    def connect(
        ...
        dispatcher.connect(receiver, signal, self.sender)
    ...
    def disconnect(self, receiver: Callable[..., T], signal: Any, **kwargs: Any) -> None:
        """
...
async def send(signal: Any, sender: Any, *args: Any, **kwargs: Any) -> List:
    ...
    :param args:
    :param kwargs:
        dont_log: 可以通过设置期待异常，在出现异常的时候不会触发 error 日志。
    :return:
    """
    responses: List = []
    ...
    for receiver in liveReceivers(getAllReceivers(sender, signal)):
        ...
        try:
            ...
            response = e
            logger.error("Error caught on signal handler: receiver", exc_info=True)
