...
class TestSignalManager:
    ...
    async def test_send_async_exception(self, mocker, signal_manager):
        ...
        assert logger_mock.called_once

    @pytest.mark.asyncio
    async def test_send_async_exception_dont_log(self, signal_manager, caplog):
        class DontLogException(Exception):
            pass

        e = DontLogException('xxx')
        ...
        res = await signal_manager.send(start_signal, x='foo', dont_log=DontLogException)
        assert (demo, e) in res
